...
interface WebEpubReaderProps {
  ...
  initialLocation?: string; // Prop para la posición inicial
  onError?: (error: Error) => void; // Prop para manejar errores
  onReady?: () => void; // Prop para notificar cuando el lector está listo
}
  ...
  useEffect(() => {
    ...
    const loadEpub = async () => {
      try {
        ...
        try {
          // Intentar cargar el libro en orden de preferencia: ArrayBuffer, Blob, URL
          if (arrayBuffer) {
            console.log('Cargando desde ArrayBuffer');
            // Intentar directamente con el ArrayBuffer
            try {
              book = ePub(arrayBuffer);
            } catch (e) {
              // Si falla, crear una URL desde un Blob
            ...
            console.log('Cargando desde Blob');
            // Convertir Blob a ArrayBuffer
            try {
              const blobArrayBuffer = await blob.arrayBuffer();
              book = ePub(blobArrayBuffer);
            } catch (e) {
              // Si falla, crear una URL desde el Blob
          ...
          if (url && (url.startsWith('http') || url.startsWith('blob:'))) {
            ...
            try {
              ...
              console.log('Intentando crear libro desde URL alternativa');
                try {
                // Intentar primero directamente con el ArrayBuffer
                const blobArrayBuffer = await epubBlob.arrayBuffer();
                book = ePub(blobArrayBuffer);
              } catch (blobError) {
                console.error('Error al crear libro desde ArrayBuffer alternativo:', blobError);
              ...
              const error = new Error(`Error al cargar el EPUB: ${errorMessage}`);
            ...
            const error = initError instanceof Error ? initError : new Error('Error desconocido');
            if (onError) {
              onError(error);
            }
            throw error;
        ...
        book.on('openFailed', (error: any) => {
          ...
          const errorObj = error instanceof Error ? error : new Error('Error al abrir el EPUB: ' + (error.message || 'Error desconocido'));
        ...
        // Notificar que el lector está listo
        if (onReady) {
          onReady();
        }      } catch (err: any) {
        console.error('Error al cargar el EPUB:', err);
        const errorObj = err instanceof Error ? err : new Error('Error al cargar el EPUB: ' + (err.message || 'Error desconocido'));
        setError(errorObj.message);
        if (onError) {
          onError(errorObj);
        }
        ...
        // Notificar el error si se proporciona el manejador
        if (onError) {
          onError(err);
        }
      }
